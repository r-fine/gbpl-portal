...
def bulk_create_previuos_records(self, model):
    previous = (
        model.objects.filter(user=self.request.user).order_by("-id")[0]
        if model.objects.filter(user=self.request.user).exists()
        else None
    )
    if previous:
        ...
        for i in range(days_diff - 1, 0, -1):
            ...
            if (
                week_day == "Saturday" and week_of_month & 1 == 1
            ):  # check if week_of_month is odd
